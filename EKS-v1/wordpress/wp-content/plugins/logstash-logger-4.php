<?php
/**
 * Plugin Name: Logstash Logger Enhanced with Click Tracking
 * Description: Envia logs detalhados para Logstash com tracking de cliques em mídia
 * Version: 4.0
 */
...
class LogstashLogger {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_log_click_event', array($this, 'handle_click_event'));
        add_action('wp_ajax_nopriv_log_click_event', array($this, 'handle_click_event'));
    }
    ...
    public function handle_click_event() {
        ...
        if (!wp_verify_nonce($_POST['nonce'], 'click_tracking_nonce')) {
        ...
        $click_data = array(
            ...
            'element_src' => esc_url_raw($_POST['element_src']),
        ...
        $file_extension = $this->get_file_extension($click_data['element_src']);
        $content_type = $this->detect_content_type($click_data['element_src']);
        
        $log_message = sprintf(
            '%s - %s [%s] "CLICK %s HTTP/1.1" 200 %d "-" "%s" [USER_ROLE:%s] [CLICK:%s:%s:%s:%s:%s]',
            $client_ip,
            $user_name,
            $timestamp,
            $click_data['page_url'],
            ...
            $user_agent,
            $user_role,
            $click_data['element_type'],
            $content_type,
            $file_extension,
            $click_data['element_id'],
            $click_data['coordinates']
        );
        
        $result = $this->send_to_logstash($log_message);
    ...
    public function add_click_tracking_script() {
        ...
        $nonce = wp_create_nonce('click_tracking_nonce');
        ?>
        ...
        (function($) {
            ...
            var clickTracker = {
                init: function() {
                    this.trackMediaClicks();
                    this.trackDownloadLinks();
                    this.trackCustomEvents();
                },
                
                trackMediaClicks: function() {
                    // Rastrear cliques em imagens
                    $('img, picture, figure').on('click', function(e) {
                        var $element = $(this);
                        var src = $element.attr('src') || $element.find('img').attr('src') || '';
                        clickTracker.logClick({
                            element_type: 'image',
                            element_src: src,
                            element_id: $element.attr('id') || '',
                            element_class: $element.attr('class') || '',
                            coordinates: clickTracker.getClickCoordinates(e)
                        });
                    });
                    
                    // Rastrear cliques em áudios
                    $('audio').on('play', function(e) {
                        ...
                        clickTracker.logClick({
                            ...
                            element_id: $element.attr('id') || '',
                            element_class: $element.attr('class') || '',
                            coordinates: 'auto-play'
                        });
                    });
                    
                    // Rastrear cliques em vídeos
                    ...
                    // Rastrear cliques em links de download
                    $('a[href*=".pdf"], a[href*=".doc"], a[href*=".docx"], a[href*=".zip"]').on('click', function(e) {
                        var $element = $(this);
                        clickTracker.logClick({
                            element_type: 'download',
                            element_src: $element.attr('href'),
                            element_id: $element.attr('id') || '',
                            element_class: $element.attr('class') || '',
                            coordinates: clickTracker.getClickCoordinates(e)
                        });
                ...
                trackDownloadLinks: function() {
                    // Rastrear downloads de arquivos específicos
                    $('a[href]').on('click', function(e) {
                        var href = $(this).attr('href');
                        var extensions = ['.pdf','.doc','.docx','.xls','.xlsx','.zip','.rar','.mp3','.mp4','.avi'];
                        
                        for (var i = 0; i < extensions.length; i++) {
                            if (href.toLowerCase().indexOf(extensions[i]) !== -1) {
                                clickTracker.logClick({
                                    element_type: 'file_download',
                                    element_src: href,
                ...
                trackCustomEvents: function() {
                    // Rastrear botões específicos
                    $('button, .btn, .button, input[type="submit"]').on('click', function(e) {
                        var $element = $(this);
                        var text = $element.text() || $element.val() || '';
                        
                        clickTracker.logClick({
                            element_type: 'button',
                            element_src: 'button://' + text.substring(0, 50),
                            element_id: $element.attr('id') || '',
                            element_class: $element.attr('class') || '',
                            coordinates: clickTracker.getClickCoordinates(e)
                        });
                ...
                logClick: function(data) {
                    ...
                    $.ajax({
                        ...
                        type: 'POST',
                        data: {
                            action: 'log_click_event',
                            nonce: data.nonce,
                            element_type: data.element_type,
                            element_src: data.element_src,
                        ...
                        error: function(xhr, status, error) {
                            console.log('Click log failed:', error);
                        }
                    });
                },
